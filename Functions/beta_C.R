...
#' R Functions provided in T. Engel, et al. Ecosphere 2021 Vol. 12 Issue 9 Pages e03745
...
# Modified functions ####
beta_C_abg <- function(x, C, extrapolation= T, interrupt=T) {
  x <- as.matrix(x)
  total <- colSums(x)
  N <- (invChat(total, C))
  C_max=C_target(x, factor=ifelse(extrapolation,2,1))
  ...
  attr(beta, "C") = C
  attr(beta, "N") = N
  return(c(gamma_value, alpha_value, beta))
  
}

beta_SN_n<-function(x, N){
  x<-as.matrix(x)
  total<-colSums(x)
  C= Chat(total,N)
